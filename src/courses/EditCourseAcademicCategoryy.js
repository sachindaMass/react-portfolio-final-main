...
const EditCourseAcademicCategoryy = () => {
    ...
    const onSubmit = async (e) => {
        e.preventDefault();
        await axios.put(`http://16.170.227.60:8080/academicEnglishSecondLang/category/${id}`, user);
        window.location.reload();
        navigate("/");
    };
    const loadUser = async () => {
        const result = await axios.get(`http://16.170.227.60:8080/academicEnglishSecondLang/category/${id}`);
        setUser(result.data);
    }
    return (
